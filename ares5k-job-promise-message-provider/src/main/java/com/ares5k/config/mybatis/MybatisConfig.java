...
public class MybatisConfig {
     ...
     */
    @Configuration
    //根据名字为 providerSqlSessionFactory的 SqlSessionFactory对指定包路径下的文件进行动态代理
    @MapperScan(
            basePackages = ProviderConfig.MAPPER_INTERFACE,
            sqlSessionFactoryRef = "providerSqlSessionFactory",
            sqlSessionTemplateRef = "providerSqlSessionTemplate")
    class ProviderConfig {

        /**
         * 提供端数据库连接地址
         */
        @Value("${spring.provider-db.datasource.url}")
        private String dbUrl;

        /**
         * 提供端数据库用户名
         */
        @Value("${spring.provider-db.datasource.username}")
        private String dbUserName;

        /**
         * 提供端数据库密码
         */
        @Value("${spring.provider-db.datasource.password}")
        private String dbPassword;

        /**
         * 提供端数据 atomikos唯一资源名
         */
        private static final String UNIQUE_RESOURCE_NAME = "provider-db";

        /**
         * 提供端数据库对应的 SQL语句路径
         */
        private static final String MAPPER_LOCATION = "provider/mapper/**/*.xml";

        /**
         * 提供端数据库对应的 Mapper接口路径
         */
        private static final String MAPPER_INTERFACE = "com.ares5k.modules.provider.mapper";

        /**
         * 用 atomikos数据源创建提供端数据库专用的 SqlSessionFactory对象
         *
         ...
         */
        @Primary
        @Bean(value = "providerSqlSessionFactory")
        public SqlSessionFactory providerSqlSessionFactory() throws Exception {
            return getSqlSessionFactory(dbUrl, dbUserName, dbPassword, UNIQUE_RESOURCE_NAME, MAPPER_LOCATION);
        }

        /**
         * 用 atomikos数据源创建提供端数据库专用的 SqlSessionTemplate对象
         *
         * @return SqlSessionTemplate对象
         * @throws Exception 异常
         * @author ares5k
         */
        @Bean(value = "providerSqlSessionTemplate")
        public SqlSessionTemplate providerSqlSessionTemplate() throws Exception {
            return new SqlSessionTemplate(providerSqlSessionFactory());
        }
    }
